...
class Predictor(object):
    ...
    def visualize(self, dets, meta, class_names, score_thres, wait=0):
        ...
        total_valid = 0
        for class_id, detections in dets.items():
            for det in detections:
                if len(det) >= 5:  # Ensure proper detection format
                    x1, y1, x2, y2, score = det[:5]
                    if score >= score_thres:
                        total_valid += 1
                        print(f"\nDetection {total_valid}:")
                        ...
                        print(f"  Width: {x2-x1:.2f}px, Height: {y2-y1:.2f}px")

        print(f"\nTotal valid detections: {total_valid}")
        print("-------------------------")
